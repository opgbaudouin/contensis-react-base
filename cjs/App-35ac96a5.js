...
var selectors$2 = require('./selectors-00e8bddc.js');
var log = require('loglevel');
var login = require('./login-bf48d1e9.js');
var awaitToJs = require('await-to-js');
require('react-hot-loader');
var RouteLoader = require('./RouteLoader-69097c81.js');
...
const fromJSLeaveImmer = (js, isOrdered = false) => {
  ...
  keys.forEach(key => {
    if (key === 'immer') {
      ...
      convertedObject.set(key, isOrdered ? reducers.fromJSOrdered(js) : immutable.fromJS(js));
    }
  });
};
...
const storeSurrogateKeys = response => {
  ...
  if (keys) selectors.reduxStore.dispatch(selectors$1.setSurrogateKeys(keys));
};
...
const navigationSagas = [effects.takeEvery(selectors.GET_NODE_TREE, ensureNodeTreeSaga)];
function* ensureNodeTreeSaga(action) {
  const state = yield effects.select();

  try {
    if (!selectors.hasNavigationTree(state)) {
      const deliveryApiVersionStatus = yield effects.select(selectors$2.selectVersionStatus);
      const project = yield effects.select(selectors$1.selectCurrentProject);
      ...
      if (nodes) {
        yield effects.put({
          type: selectors.SET_NODE_TREE,
          nodes
        });
      } else {
        yield effects.put({
          type: selectors.GET_NODE_TREE_ERROR
        });
      }
    }
  } catch (ex) {
...
function* setRouteSaga(action) {
  yield effects.put({
    type: selectors$1.CALL_HISTORY_METHOD,
    payload: {
      method: 'push',
...
function* getRouteSaga(action) {
  ...
  try {
    ...
    const state = yield effects.select();
    const routeEntry = selectors$1.selectRouteEntry(state); // const routeNode = selectCurrentNode(state);

    const currentPath = action.path; //selectCurrentPath(state);

    const deliveryApiStatus = selectors$2.selectVersionStatus(state);
    ...
    if (!isPreview && (appsays && appsays.customRouting || staticRoute && !staticRoute.route.fetchNode || routeEntry && action.statePath === action.path)) {
      ...
      } else yield effects.call(setRouteEntry, routeEntry && routeEntry.toJS(), (yield effects.select(selectors$1.selectCurrentNode)), (yield effects.select(selectors$1.selectCurrentAncestors)));
    } else {
      // Handle homepage
      if (isHome) {
        ...
        if (isPreview) {
          ...
          if (setContentTypeLimits && pathNode && pathNode.entry && pathNode.entry.sys && pathNode.entry.sys.id) {
            const {
              fields,
              linkDepth
            } = selectors$1.findContentTypeMapping(ContentTypeMappings, pathNode.entry.sys.id) || {};
    ...
    if (pathNode && pathNode.entry && pathNode.entry.sys && pathNode.entry.sys.id) {
      ...
      const {
        entryMapper
      } = selectors$1.findContentTypeMapping(ContentTypeMappings, entry.sys.contentTypeId) || {};
    ...
    if (!selectors.hasNavigationTree(state) && (doNavigation === true || doNavigation.tree)) if (typeof window !== 'undefined') {
      yield effects.put({
        type: selectors.GET_NODE_TREE,
...
function* setRouteEntry(entry, node, ancestors, siblings, entryMapper, notFound = false) {
  ...
  const mappedEntry = currentEntryId === entrySys.id && currentEntryLang === entrySys.language ? (yield effects.select(selectors$1.selectMappedEntry) || immutable.Map()).toJS() : yield mapRouteEntry(entryMapper, { ...node,
    entry,
    ancestors,
    siblings
  });
...
//# sourceMappingURL=App-35ac96a5.js.map
